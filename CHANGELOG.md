...
- **MiniMax endpoint 补 capabilities**：M3 系列（M3 / M3-highspeed）和 M2.7 系列标 `text + tools + reasoning`，M2 / M2.1 / M2.5 系列标 `text + tools`，M2-her / Text-01 标 `text only`（角色对话 / 老文本模型不参与 tool call）。
...
- **测试覆盖**：新增 MiniMax 适配回归组（`providers-schema`、`service-presets-regression`、`provider.test`），锁定 stream=true / compat / capabilities / temperatureRange / piProvider 五项不变量。
